...
import spring_data_bookshop.entities.*;
...
@Service
public class ServiceImpl implements SeedService {
    ...
    @Autowired
    public ServiceImpl(AuthorRepository authorRepository, CategoryRepository categoryRepository, BookRepository bookRepository, AuthorService authorService, CategoryService categoryService) {
    ...
    public void seedBooks() throws IOException {
        ...
        Files.readAllLines(Path.of( BOOKS_FILE_PATH))
                .stream()
                .filter(s -> !s.isBlank())
                .map(this::getBookObject).forEach(bookRepository::save);
    ...
    private Book getBookObject(String line) {
        ...
        Book book = new Book(title,editionType,price, copies,
                publishDate, ageRestriction, author, categories);
